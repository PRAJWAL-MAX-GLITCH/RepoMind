...
test-coverage:
	rm -rf "$(TEST_LOCAL_DATA_DIR)"/*
	PGPT_HOME=$(TEST_PGPT_HOME) PYTHONPATH=. uv run pytest tests --cov repomind --cov-report term --cov-report=html --cov-report xml --junit-xml=tests-results.xml

black:
	uv run black . --check

ruff:
	uv run ruff check repomind tests scripts

format:
	uv run black .
	uv run ruff check repomind tests scripts --fix
...
mypy:
	...
	uv run dmypy check repomind scripts
...
dev-windows:
	uv run repomind serve --reload

dev:
	PYTHONUNBUFFERED=1 uv run repomind serve --reload --host 0.0.0.0 --port 8080
...
prod-run:
	...
	@wheel="$$(ls -t $(PROD_DIST_DIR)/repomind-*.whl 2>/dev/null | head -n 1)"; \
	if [ -z "$$wheel" ]; then \
...
api-docs:
	PGPT_PROFILES=mock uv run python scripts/extract_openapi.py repomind.main:app --out fern/openapi/openapi.json
...
celery:
	PGPT_WORKER_MODE=worker uv run repomind worker

flower:
	PGPT_WORKER_MODE=flower uv run repomind worker
