...
def _delimiter_reader() -> IngestionReader:
    try:
        from repomind.parsers.readers.text.delimiter_reader import (
            DelimiterTextReader,
        )
...
def _email_reader() -> IngestionReader:
    try:
        from repomind.parsers.readers.text.email_reader import (
            EmailTextReader,
        )
    except ImportError as e:
        raise ImportError(
...
def _html_reader() -> IngestionReader:
    try:
        from repomind.parsers.readers.text.html_reader import (
            HtmlReader,
        )
    except ImportError as e:
        raise ImportError(
...
def _text_reader() -> IngestionReader:
    try:
        from repomind.parsers.readers.text.text_reader import (
            TextReader,
        )
    except ImportError as e:
        raise ImportError(
