...
from repomind.utils.concurrency.registry import create_semaphore_manager
from repomind.utils.concurrency.semaphore_manager import SemaphoreManager
from repomind.models.llm.llm_helper import supports_images
from repomind.components.multimodality.image_handler import describe_image
from repomind.rag.prompts.prompt_builder import PromptBuilderService
from repomind.core.di import get_global_injector
from repomind.core.settings.settings import TransformationReadersSettings
...
class ExtractSlideContentFromImageAndTextTransform(TransformComponent):
    """Extract complete slide content using both OCR text and the full slide image."""
        ...
        **kwargs: Any,
        ...
        tasks = [
            semaphore_manager.execute(
                task_func=self._extract_slide_content,
                priority=idx,
                node=node,
                vlm=vlm,
                prompt_builder=prompt_builder,
            ...
            else self._extract_slide_content(
                node=node,
                vlm=vlm,
                prompt_builder=prompt_builder,
                **kwargs,
    ...
    async def _extract_slide_content(
        ...
        **kwargs: Any,
        ...
        slide_image_b64 = node.metadata.get("slide_image")
        if not slide_image_b64:
            return node

        try:
            image_block = ImageBlock(
                image=base64.b64decode(slide_image_b64),
                image_mimetype="image/png",
            )
        except Exception as e:
            logger.warning("Failed to decode slide image metadata: %s", e)
            self._clean_slide_metadata(node)
            return node

        current_content = node.get_content(MetadataMode.NONE)
        cleaned_context = self._clean_text_context(current_content)

        prompt = prompt_builder.create_pptx_slide_fusion_prompt(
            extracted_text=cleaned_context
        )
        ...
        try:
            ...
            logger.warning("Failed to extract slide content from image+text: %s", e)
        finally:
            self._clean_slide_metadata(node)

        return node

    def _clean_text_context(self, content: str) -> str:
        # Remove embedded data-url images to avoid huge prompt payloads.
        stripped = re.sub(self._image_regex, "", content)
        stripped = re.sub(r"\n{3,}", "\n\n", stripped).strip()
        return stripped[:_MAX_TEXT_CONTEXT_CHARS]

    @staticmethod
    def _clean_slide_metadata(node: BaseNode) -> None:
        if "slide_image" in node.metadata:
            del node.metadata["slide_image"]
        if "zones" in node.metadata:
            del node.metadata["zones"]
