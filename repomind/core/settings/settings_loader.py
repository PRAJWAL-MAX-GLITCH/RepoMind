...
from repomind.core.settings.yaml import load_yaml_with_envvars

logger = logging.getLogger(__name__)

_settings_folder_str = os.environ.get("REPOMIND_SETTINGS_FOLDER", str(PROJECT_ROOT_PATH))
...
active_profiles: list[str] = unique_list(
    ...
    + [
        item.strip()
        for item in os.environ.get("REPOMIND_PROFILES", "").split(",")
        if item.strip()
    ]
    + _test_profile
)
...
def discover_models_from_environment(
    ...
    REPOMIND_MODELS_<MODEL_ID>_<PARAMETER>[__<NESTED_PARAM>]

    Examples:
        REPOMIND_MODELS_GPT4_API_KEY=sk-123 ->
            {"gpt4": {"api_key": "sk-123", "name": "gpt4"}}
        REPOMIND_MODELS_CLAUDE_CONFIG__TEMPERATURE=0.7 ->
    ...
    for key, value in environ.items():
        if key.startswith("REPOMIND_MODELS_") and key.count("_") >= 3:
