...
from repomind.ingestion.ingest.utils import get_extension, get_file_name
class S3Helper:
    pass
from repomind.api.server.ingest.ingest_service import IngestService
from repomind.api.server.ingest.model import IngestedDoc
from repomind.api.server.utils.artifact_input import IngestableArtifactType, UriArtifact
from repomind.api.server.utils.auth import authenticated
from repomind.api.server.utils.callback import BaseCallbackInput
from repomind.core.settings.settings import settings

try:
    from repomind.celery.celery import celery_app
    from repomind.celery.model import Task, TaskStatus
...
class IngestResponse(BaseModel):
    ...
    model: Literal["repomind"] = Field(
        default="repomind", description="Model identifier, always 'repomind'"
    )
...
def list_ingested(
    ...
    return IngestResponse(object="list", model="repomind", data=ingested_documents)
...
if _CELERY_AVAILABLE:
    ...
    def delete_ingested_async(body: DeleteIngestedDocumentAsyncBody) -> Task:
        ...
        from repomind.celery.task_helper import IngestionTaskHelper
