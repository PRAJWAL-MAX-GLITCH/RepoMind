...
from repomind.artifact_index.base_artifact_index import IndexNotReadyException
from repomind.celery.tasks.ingestion import delete_ingested_task, vector_index_task
from repomind.components.storage.s3_helper import S3Helper
from repomind.api.server.ingest.ingest_router import (
    DeleteIngestedDocumentAsyncBody,
    DeleteIngestedDocumentBody,
    IngestAsyncBody,
    IngestBody,
)
from repomind.api.server.utils.artifact_input import FileArtifact, UriArtifact
...
def test_delete_already_ingested_document(mock_setup, test_bodies):
    ...
    with (
        patch(
            "repomind.celery.tasks.ingestion.delete_tasks.celery_app", mock_celery
        ),
        patch(
            "repomind.celery.tasks.ingestion.delete_tasks.get_global_injector",
...
def test_delete_scheduled_when_ingestion_will_run(mock_setup, test_bodies):
    ...
    with (
        patch(
            "repomind.celery.tasks.ingestion.extraction_tasks.celery_app",
            mock_celery,
        ),
        patch(
            "repomind.core.di.get_global_injector",
            return_value=mock_injector,
        ),
    ):
...
def test_cleanup_removes_temporary_s3_file():
    ...
    with patch(
        ...
        from repomind.celery.tasks.ingestion.extraction_tasks import (
...
def test_cleanup_remove_temporary_with_failed_s3_file(mock_setup):
    ...
    with (
        ...
        patch(
            "repomind.celery.task_helper.IngestionTaskHelper.is_ingestion_cancel_task_scheduled",
