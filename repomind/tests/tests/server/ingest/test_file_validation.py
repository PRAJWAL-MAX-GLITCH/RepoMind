...
from repomind.ingestion.ingest.progress.errors import IngestionValidationErrors
from repomind.ingestion.ingest.utils import FileInfo, get_file_info
...
def test_get_file_info_valid_pdf():
    ...
    with patch(
        "repomind.ingestion.ingest.utils.get_filesize"
    ) as mock_filesize, patch(
        "repomind.ingestion.ingest.utils.get_guest_mime_type"
    ) as mock_guest_mime, patch(
        "repomind.ingestion.ingest.utils.get_actual_mime_type"
    ) as mock_actual_mime, patch(
        "repomind.ingestion.ingest.utils.detect_encoding"
    ) as mock_encoding, patch(
        "repomind.ingestion.ingest.utils.extract_config"
    ) as mock_config:
