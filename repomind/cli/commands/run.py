...
def _start_server_subprocess() -> "subprocess.Popen[bytes]":
    s = settings().server
    repomind_bin = shutil.which("repomind")
    base_cmd = (
        [repomind_bin, "serve"]
        if repomind_bin
        else [sys.executable, "-m", "repomind", "serve"]
    )
...
def _inject_opencode_env(model: str | None = None) -> None:
    ...
    default_model = f"RepoMind - {default_model}"
    ...
    config_path = config_dir / "repomind.json"
...
def run_command(
    ...
    Pass extra arguments to the app after --:
        ...
        repomind run claude-code -- --resume abc123
    """
    ...
    if detach:
        ...
        log_dir = Path.home() / ".repomind" / "runs"
        log_dir.mkdir(parents=True, exist_ok=True)
