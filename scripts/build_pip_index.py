...
def build_index(package_dir: Path, output_dir: Path, extra: str) -> None:
    ...
    write_text(
        ...
        render_page("RepoMind package files", "\n    ".join(artifact_links)),
    )

    write_text(
        simple_dir / "index.html",
        render_page("RepoMind package index", "\n    ".join(package_links)),
    )
    write_text(
        output_dir / "index.html",
        render_page(
            "RepoMind package index",
            (
                "<p>Install RepoMind with <code>uv tool install --find-links "
                "https://wheels.privategpt.dev/packages/ "
                f"&quot;repomind[{html.escape(extra)}]&quot;</code>.</p>"
            ),
        ),
    )
