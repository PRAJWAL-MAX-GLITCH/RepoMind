#!/usr/bin/env python3
"""Generate the Homebrew formula for repomind."""
...
def render_formula(
  ...
  def install
    (bin/"repomind").write <<~SH
      #!/bin/bash
      exec "#{{Formula["uv"].opt_bin}}/uv" tool run --python "#{{Formula["python@3.11"].opt_bin}}/python3.11" --find-links "{package_files_url}" --from "repomind[{extra}]=={version}" repomind "$@"
    SH
  end

  test do
    script = (bin/"repomind").read
    assert_match "repomind[{extra}]=={version}", script
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--homepage",
        default="https://github.com/zylon-ai/repomind",
    )
